...
class Boundary {
  constructor(props) {
    ...
    this.orderType = ORDER_TYPE.VERTICAL;
  ...
  create(options = {}, callbackDragBoundary = () => { },  callbackDragConnection = ()=>{}) {
    let { id, x, y, name, description, member, width, height, parent, mandatory, repeat, isMenu, show, orderType} = options;
    ...
    this.orderType = orderType || ORDER_TYPE.VERTICAL;
    ...
    group.append("foreignObject")
      ...
      .html(`
          <div class="boundary_header" style="pointer-events: all;
            width: 100%;
            line-height: ${BOUNDARY_ATTR_SIZE.HEADER_HEIGHT}px;
            font-size: ${BOUNDARY_ATTR_SIZE.HEADER_FONT_SIZE}px;
  ...
  updateSize() {
    if (this.orderType === ORDER_TYPE.VERTICAL) {
      this.updateSizeVertical();
    } else {
      this.updateSizeHorizontal();
    }
  }
   ...
   */
  updateSizeVertical() {
    const padding = BOUNDARY_ATTR_SIZE.PADDING;
    const childMargin = BOUNDARY_ATTR_SIZE.CHILD_MARGIN;
    const headerHeight = BOUNDARY_ATTR_SIZE.HEADER_HEIGHT + 1;
    let orderObject = 0;
    let wBoundary = this.member.length < 0 ? BOUNDARY_ATTR_SIZE.BOUND_WIDTH : 0;
    let hBoundary = this.member.length < 0 ? BOUNDARY_ATTR_SIZE.BOUND_HEIGHT : 0;
    ...
    wBoundary += padding*2;
    hBoundary = padding*2 + headerHeight + hBoundary + childMargin*(orderObject-1);
  ...
  updateSizeHorizontal() {
    ...
    this.member.forEach((mem, index) => {
      if (mem.show) {
        if (mem.type === OBJECT_TYPE.BOUNDARY) {
          let boundaryObj = _.find(this.dataContainer.boundary, {"id": mem.id});
          boundaryObj.updateSizeHorizontal();
        }

        const { width, height } = this.objectUtils.getBBoxObject(`#${mem.id}`);
        orderObject++;
        wBoundary += width;
        if (height  > hBoundary) hBoundary = height;
      }
    });

    wBoundary += padding*2 + (orderObject-1)*childMargin;
    hBoundary += padding*2 + headerHeight;
  ...
  reorderPositionMember(pos) {
    if (this.orderType === ORDER_TYPE.VERTICAL) {
      this.reorderPositionMemberVertical(pos);
    } else {
      this.reorderPositionMemberHorizontal(pos);
    }
  }
   ...
   * @param position
   */
  reorderPositionMemberVertical(pos) {
    let orderObject = 0;
    let hBeforeElements = BOUNDARY_ATTR_SIZE.HEADER_HEIGHT + 1;
    const childMargin = BOUNDARY_ATTR_SIZE.CHILD_MARGIN;
    const padding = BOUNDARY_ATTR_SIZE.PADDING;
    ...
    this.member.forEach(mem => {
      if (mem.show) {
        ...
        let position = { x: pos.x + padding, y: pos.y + hBeforeElements + childMargin*orderObject + padding };
        let memObj = {};
   ...
   * @param position
   */
  reorderPositionMemberHorizontal(pos) {
    let orderObject = 0;
    let hBeforeElements = BOUNDARY_ATTR_SIZE.HEADER_HEIGHT + 1;
    const childMargin = BOUNDARY_ATTR_SIZE.CHILD_MARGIN;
    const padding = BOUNDARY_ATTR_SIZE.PADDING;
    let wBound = this.member.length < 0 ? BOUNDARY_ATTR_SIZE.BOUND_WIDTH : 0;
    ...
    this.member.forEach(mem => {
      if (mem.show) {
        ...
        // Vertex position center of boundary
        let position = { x: pos.x + padding + wBound + childMargin*orderObject, y: pos.y + hBeforeElements + padding };
        let memObj = {};
        if (mem.type === OBJECT_TYPE.VERTEX) {
          memObj = _.find(this.dataContainer.vertex, { "id": mem.id });
        } else {
 ...
 */
  ...
  applyFontSize() {
    d3.select(`#${this.id} .boundary_header`)
    .style('line-height', `${BOUNDARY_ATTR_SIZE.HEADER_HEIGHT}px`)
    .style('font-size', `${BOUNDARY_ATTR_SIZE.HEADER_FONT_SIZE}px`);
  }
}
