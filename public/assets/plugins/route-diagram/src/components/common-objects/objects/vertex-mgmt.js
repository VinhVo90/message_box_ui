...
import CopyValueMenu from '../menu-context/copy_value_menu';
...
class VertexMgmt {
	...
	initialize() {
		...
		new VertexMenu({
			selector: `.${this.selectorClass} .content_header_name`,
			vertexMgmt: this,
    ...
    new CopyValueMenu({
      selector: `.${this.selectorClass} .property`,
    });
	...
	startDrag(main) {
      ...
      const eventX = d3.mouse(d3.select(`#${main.svgId}`).node())[0];
      const eventY = d3.mouse(d3.select(`#${main.svgId}`).node())[1];

      const id = $(this).closest('g').attr('id');
      const d = main.dragingObject = _.find(main.dataContainer.vertex, {id: id});
			...
			main.edgeMgmt.emphasizePathConnectForVertex(d);

			d.startX = d.x;
			d.startY = d.y;
      main.offsetX = eventX - d.x;
      main.offsetY = eventY - d.y;
   ...
   */
	...
	reRenderContentInsideVertex(vertex, isEffectToParent = true) {
		...
		vertex.generateContent(this.handleDragVertex, this.edgeMgmt.handleDragConnection);
  ...
  setPositionObjectJustInSvg(event) {
		// Limit left
    let x = event.x - this.offsetX < PADDING_POSITION_SVG.MIN_OFFSET_X ? PADDING_POSITION_SVG.MIN_OFFSET_X : event.x - this.offsetX;
    const y = event.y - this.offsetY < PADDING_POSITION_SVG.MIN_OFFSET_Y ? PADDING_POSITION_SVG.MIN_OFFSET_Y : event.y - this.offsetY;
    
		// limit right
		if (!checkModePermission(this.dragingObject.viewMode.value, 'horizontalScroll')) {
			let limitWidth = $(`#${this.dragingObject.svgId}`).width();
			let {width} = getBBoxObject(`#${this.dragingObject.id}`);
