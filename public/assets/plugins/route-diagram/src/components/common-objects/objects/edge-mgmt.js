...
import {
  CONNECT_TYPE, ACTION_TYPE, OBJECT_TYPE, LINE_STYLE,
...
class EdgeMgmt {
  ...
  dragPointEnded(main) {
    return function () {
      ...
      if ($target.attr('class') != undefined && $target.attr('class').indexOf('drag_connect') != -1) {
        ...
        const newPoint = main.objectUtils.getCoordProp(targetObj, prop, pointType);
  ...
  startConnect(main) {
    return function () {
      ...
      const src = main.objectUtils.getCoordProp(obj, prop, CONNECT_TYPE.OUTPUT);
  ...
  drawConnect(main) {
    return function () {
      if (main.isCreatingEdge) {
        ...
        // if ($target.attr('class') != undefined && $target.attr('class').indexOf('drag_connect') != -1) {
        //   const vertexId = d3.select(d3.event.sourceEvent.target.parentNode).attr('id');
        //   pathStr = createPath(main.tmpSource, { vertexId: vertexId, x: x2, y: y2 }, LINE_STYLE.POLYLINES, true);
        // } else {
        //   pathStr = createPath(main.tmpSource, { x: x2, y: y2 }, LINE_STYLE.STRAIGHT, true);
        // }
  ...
  endConnect(main) {
    return function () {
      ...
      if ($target.attr('class') != undefined && $target.attr('class').indexOf('drag_connect') != -1
        ...
        const des = main.objectUtils.getCoordProp(obj, prop, CONNECT_TYPE.INPUT);
  ...
  */
  updatePathConnectForVertex(vertex) {
    ...
    arrSrcPaths.forEach((src) => {
      ...
      const newPos = this.objectUtils.getCoordProp(vertex, prop, CONNECT_TYPE.OUTPUT);
    ...
    arrDesPaths.forEach((des) => {
      ...
      const newPos = this.objectUtils.getCoordProp(vertex, prop, CONNECT_TYPE.INPUT);
  ...
  emphasizePathConnectForVertex(vertex, isEffectFromParent = false) {
    ...
    if (!isEffectFromParent) {
      d3.selectAll('.emphasizePath1').classed('emphasizePath1', false);
