...
import {
  LINE_TYPE, OBJECT_TYPE, ACTION_TYPE, NOTE_TYPE, CONNECT_TYPE, LINE_STYLE,
...
class Edge {
  constructor(props) {
    ...
    this.source;
    this.target;
    this.lineStyle = LINE_STYLE.STRAIGHT;
  ...
  create(sOptions) {
    ...
    const pathStr = createPath(this.source, this.target, this.lineStyle, this.useMarker, this.lineWidth);
    ...
    group.append('path')
      ...
      .attr('stroke-width', this.lineWidth < 9 ? 9 : this.lineWidth)
    ...
    group.append('path')
      ...
      .attr('fill', 'none')
      .attr('focusable', true)
      .attr('marker-end', this.useMarker === 'Y' ? `url(#arrow${this.id})` : '') // Make arrow at end path
      .attr('stroke-dasharray', this.lineType === LINE_TYPE.SOLID ? '0 0' : '3 3'); 
  ...
  updatePathConnect(sOptions) {
    if (sOptions) {
      ...
      let newPosition = this.objectUtils.getCoordProp(tmpObject, this.source.prop, CONNECT_TYPE.OUTPUT);
      this.source.x = newPosition.x;
      this.source.y = newPosition.y;

      // for target
      tmpObject = _.find(vertices, { id: this.target.vertexId });
      newPosition = this.objectUtils.getCoordProp(tmpObject, this.target.prop, CONNECT_TYPE.INPUT);
      this.target.x = newPosition.x;
      this.target.y = newPosition.y;
    }

    const { source, target } = this;
    const pathStr = createPath(source, target, this.lineStyle, this.useMarker, this.lineWidth);
  ...
  setStatusEdgeOnCurrentView() {
    ...
    const addressBarHeight = $('#addressBar').length > 0 ? $('#addressBar')[0].getBoundingClientRect().height : 0;
  ...
  setLineStyle(style) {
    ...
    // Get DOM and update attribute
    d3.selectAll(`#${this.id}`).attr('d', pathStr);
    d3.select(`#${this.edgePathId}`).attr('d', pathStr);
  }
  ...
  setUseMarker(flag) {
    ...
    .attr('d', pathStr)
    .attr('marker-end', flag === 'Y' ? `url(#arrow${this.id})` : '');
  }
  ...
  emphasize() {
    ...
    this.lineWidth === 1 ? path.classed('emphasizePath1', true) : path.classed('emphasizePath', true);
  ...
  updateConnector(dropVertexId, pointType, propId) {
    ...
    const newPoint = this.objectUtils.getCoordProp(targetObj, propId, pointType);
