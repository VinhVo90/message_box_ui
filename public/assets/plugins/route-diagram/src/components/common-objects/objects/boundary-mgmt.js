...
class BoundaryMgmt {
  ...
  setFontSize(fontSize) {
    const { headerHeight, headerFontSize } = this.calculateHeaderSize(fontSize);
    BOUNDARY_ATTR_SIZE.HEADER_HEIGHT = headerHeight;
    BOUNDARY_ATTR_SIZE.HEADER_FONT_SIZE = headerFontSize;

    for (let i = 0; i < this.dataContainer.boundary.length; i += 1) {
      const boundary = this.dataContainer.boundary[i];
      boundary.applyFontSize();
    }
  }

  calculateHeaderSize(fontSize = 0) {
    let headerHeight = 38;
