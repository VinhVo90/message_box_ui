...
class ObjectUtils {
   ...
   */
	getCoordPropRelativeToParent(info, prop, type) {
      ...
      const $property = $(`[prop="${prop}"]:not(.drag_connect)`)[0];
      axisY = axisY + $property.offsetTop + (($property.offsetHeight-1)/2);
  ...
  getCoordProp(info, prop, type) {
    ...
    let axisY = y;
		...
		if (prop.indexOf('boundary_title') != -1) {
			...
			return {
				x: type === CONNECT_TYPE.OUTPUT ? axisX + info.width : axisX,
				y: axisY
			}
		} else if (prop.indexOf('title') != -1) {
			axisY = axisY + VERTEX_ATTR_SIZE.HEADER_HEIGHT / 2;
			...
			return {
				x: type === CONNECT_TYPE.OUTPUT ? axisX + VERTEX_ATTR_SIZE.GROUP_WIDTH : axisX,
				y: axisY
			}
		} else{
      const $property = $(`[prop="${prop}"]:not(.drag_connect)`)[0];
   ...
   */
    ...
    const { x: xSrc, y: ySrc } = obj;
		...
		for (const mem of memberAvailable) {
			...
			if (parentObj.orderType === ORDER_TYPE.VERTICAL) {
        if (y > ySrc)
				  break;
			} else if (x > xSrc) {
        break;
   ...
   */
	...
	onContainerSvgScroll(pSvgId, edgeMgmt, arrDataContainer) {
		...
		// 	const {x: propX, y: propY} = this.getCoordProp(obj, prop, CONNECT_TYPE.OUTPUT);
		// 	e.source.x = propX;
		// 	e.source.y = propY;
		...
		// 	const obj = _.find(vertices, {'id': id});
		// 	const {x: propX, y: propY} = this.getCoordProp(obj, prop, CONNECT_TYPE.INPUT);
		// 	e.target.x = propX;
		// 	e.target.y = propY;
	...
	updatePathConnectOnWindowResize(edgeMgmt, arrDataContainer) {
		...
		edges.forEach(e => {
			...
			const {x: newSX, y: newSY} = this.getCoordProp(srcObj, propSrc, CONNECT_TYPE.OUTPUT);
			e.source.x = newSX;
			e.source.y = newSY;

			const desObj = _.find(vertices, {'id': idDes});
			const {x: newDX, y: newDY} = this.getCoordProp(desObj, propDes, CONNECT_TYPE.INPUT);
			e.target.x = newDX;
			e.target.y = newDY;
   ...
   */
	updatePositionRectConnect(arrProp, vertex) {
		for (var i = 0; i < arrProp.length; i++) {
			...
			if (prop != null) {
        const newY = $(`#${vertex.id} .vertex_data .property[prop="${prop}"]`)[0].offsetTop;
